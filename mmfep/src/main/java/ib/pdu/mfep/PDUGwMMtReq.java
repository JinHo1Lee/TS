...
public class PDUGwMMtReq extends PDUGwMtReq implements Serializable{
	...
	public String toString() {		
		...
		for (int i=0; i<recipients.size(); i++){
			...
			outStr += String.format("tsCode:%s, routeGroup:%s, clientCode:%s, netId:%d, msgType:%s, carrierKey:%s, clientMsgKey:%s, rsMsgKey:%s, rsID:%s, priority:%s, CallBack:%s, Recpient:%s, npsend:%s, ttl:%s",
									 tsCode, routeGroup, clientCode, carrier, msgType, carrierKey, clientMsgKey, rsMsgKey, rsID, priority, callBack, recpientFormat, recipients.get(0).getRecipientNpSend(), ttl);
			if (i>0)outStr +="\n";
		}
		return outStr;
	}
	...
	public PDUGwMMtReq (byte[] buffer) throws PduException, UnsupportedEncodingException{
		...
		tc = ByteUtil.getint(buffer, pos);
		pos += PDUConst.SIZE_INT;
		if (tc == PDUConst.TAG_CODE_SUBJECT){
	...
	public PDUGwMtRes makePduGwRes(int resCode){
		...
		pduRes.setPduType(PDUConst.PDU_TYPE_GW_MT_MMS_RES);
