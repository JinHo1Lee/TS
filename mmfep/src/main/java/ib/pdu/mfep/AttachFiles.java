...
public class AttachFiles extends ArrayList<AttachFileInfo> implements Serializable{
	...
	public ByteBuffer encode(int length) throws PduException {
		...
		for (int i = 0; i < this.size(); i++) {
			AttachFileInfo attachFileInfo = this.get(i);
	...
	public ArrayList<AttachFileInfo> getRecipientInfoList() {
		return this;
	}
