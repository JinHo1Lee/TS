...
public class TcpUtil {
	 ...
	 * { ByteArrayOutputStream bout = new ByteArrayOutputStream(); int bcount =
	 * 0; byte buf[] = new byte[2048]; int read_retry_count = 0; while(bcount <
	 * len) { int n = in.read(buf, 0, len - bcount >= 2048 ? 2048 : len -
	 * bcount); if(n == -1) throw new
	 * IOException("inputstream has returned an unexpected EOF"); if(n == 0 &&
	 * ++read_retry_count == 10) throw new
	 * IOException("inputstream-read-retry-count( 10) exceed !"); if(n != 0) {
	 * bcount += n; bout.write(buf, 0, n); } } bout.flush(); return
	 * bout.toByteArray(); }
	 */
	...
	/*
	 * public static void read_data_skip(InputStream in, int len) throws
	 * Exception { int bcount = 0; byte buf[] = new byte[2048]; int
	 * read_retry_count = 0; while(bcount < len) { int n = in.read(buf, 0, len -
	 * bcount >= 2048 ? 2048 : len - bcount); if(n == -1) throw new
	 * IOException("inputstream has returned an unexpected EOF"); if(n == 0 &&
	 * ++read_retry_count == 10) throw new
	 * IOException("inputstream-read-retry-count( 10) exceed !"); if(n != 0) {
	 * bcount += n; } } }
	...
	public static void read_data_skip(InputStream in, int len) throws Exception {
		...
		while (skipcount < len) {
			...
			if (n > 0)
				...
				throw new IOException("inputstream-skip-retry-count( 10) exceed, expected:" + len + ", but actual:" + skipcount);
		}
	...
	public static byte[] read_data(InputStream in) throws Exception {
	...
	public static String read_line(InputStream in) throws Exception {
		ByteArrayOutputStream bout = new ByteArrayOutputStream();
		boolean eof = false;
		int b;
		do {
	...
	public static final String getstring(InputStream in) throws Exception {
		...
		if (len > 0)
			return new String(read_data(in, len));
